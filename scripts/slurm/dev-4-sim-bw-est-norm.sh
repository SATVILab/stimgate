...
#SBATCH --nodes=1
#SBATCH --ntasks=5
#SBATCH --job-name="dev-4-est"
#SBATCH --partition=ada

set -euo pipefail

chunk_index="${1:-${SIM_GRID_CHUNK_INDEX:-${SLURM_ARRAY_TASK_ID:-1}}}"
n_chunks="${SIM_GRID_N_CHUNKS:-1}"
project_root="${PROJECT_ROOT:-${SLURM_SUBMIT_DIR:-$(pwd)}}"
qmd_file="${SIM_GRID_QMD_FILE:-analysis/4-sim-bw-est-norm.qmd}"
...
if [[ -z "$analysis_run_id" && -n "${SLURM_ARRAY_JOB_ID:-}" ]]; then
  analysis_run_id="analysis-4-slurm-${SLURM_ARRAY_JOB_ID}"
elif [[ -z "$analysis_run_id" && "$n_chunks" == "1" ]]; then
  analysis_run_id="analysis-4-slurm-${SLURM_JOB_ID:-$(date -u +%Y%m%dT%H%M%S)}"
