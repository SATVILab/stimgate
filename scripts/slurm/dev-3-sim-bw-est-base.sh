...
qmd_file="${SIM_GRID_QMD_FILE:-analysis/3-sim-bw-est-base.qmd}"
...
if [[ -z "$analysis_run_id" && -n "${SLURM_ARRAY_JOB_ID:-}" ]]; then
  analysis_run_id="analysis-3-slurm-${SLURM_ARRAY_JOB_ID}"
elif [[ -z "$analysis_run_id" && "$n_chunks" == "1" ]]; then
  analysis_run_id="analysis-3-slurm-${SLURM_JOB_ID:-$(date -u +%Y%m%dT%H%M%S)}"
