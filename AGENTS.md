...
- Use `@import` or `@importFrom` directives in roxygen comments;
  ...
  exceptions are `ggplot2` (imported wholesale via `#' @import ggplot2`
  in `R/misc.R`) and
...
- Exceptions: `ggplot2` is imported wholesale via `#' @import ggplot2`
  in `R/misc.R`, so `ggplot2` functions and
...
6.  **Standardised simulation and plotting controls across analysis QMDs
    ...
    - YAML headers declare
      ...
      chunking parameters).
    - Setup chunks initialise `run_simulations` (defaulting to `FALSE`
      in interactive execution) and `run_plots` (defaulting to `TRUE` in
      interactive execution) via `.as_flag(.get_qmd_param_env(...))`.
    - Environment variables `RUN_SIMULATIONS` and `RUN_PLOTS` override
      YAML parameters and interactive default values.
    - Expensive simulation chunks are guarded with
      `if (isTRUE(run_simulations))`, and plotting chunks are guarded
      with `if (isTRUE(run_plots))`.
