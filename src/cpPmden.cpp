/*
 * Provenance: Derived from FAUST (https://github.com/RGLab/FAUST, cpPmden.cpp).
 * Originally ported from R package ftnonpar 0.1-88 (pmden).
 ...
 * However, these values appear to be coerced to double when the library is loaded for use in an R session.
 ...
 * (Don't have matrix row structure for cpPmden, contrary to R implementation)
 */
...
stringInfo cpPmden(const std::vector<double>& xIn) {
  ...
  for (auto i : fdist_y) {
    fdist_y[(int)i] = fdist_y[(int)i]/double((nsamp-1));
  }
  ...
  while (stillRepeating && (cpPmden_iter++ < cpPmden_max_iter)) {
    ...
    for (auto i=0; i < (int)(nsamp-2); i++) {
      if (x_string[(i+1)] != x_string[i]) {
	//capture the jump points on the first pass, to remove double-checking later
	if (first_jump == -1) {
	  first_jump = i;
	}
	last_jump = i;
    ...
    first_jump = -1;
    last_jump = -1;
    //next, linear interpolation between knots of the taut string
