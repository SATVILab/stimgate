...
validate_priority_mapping() {
  ...
  if grep -Fxq 'Priority mapping status: pending' "$file"; then
    ...
    [[ "$pending_count" == "1" ]] || die "$file must declare the pending Priority status exactly once"
    for common in P0 P1 P2 P3; do
      provider="$(priority_value "$file" "$common")"
      [[ -z "$provider" ]] || die "$file mixes a pending Priority status with a $common mapping"
    done
    return 0
  fi
  ...
  [[ -z "$duplicate" ]] || die "$file Priority mapping is not one-to-one"
}
...
validate_styles() {
  ...
  case "$value" in
    ...
    *) die "$file has unsupported Issue write-up style '$value'; use direct, tidy or unrestricted" ;;
  esac
  value="$(table_value "$file" "Issue prose style")"
  case "$value" in
    ""|natural-direct) ;;
    *) die "$file has unsupported Issue prose style '$value'; use natural-direct" ;;
  esac
}
...
validate_project_file() {
  ...
  if [[ "$expected_mode" == "project" ]]; then require_table_value "$file" "Project key" >/dev/null; fi
  ...
  grep -Eq '^## Field locations[[:space:]]*$' "$file" || die "$file is missing Field locations"
  grep -Eq '^\|[[:space:]]*Priority[[:space:]]*\|' "$file" || die "$file does not declare the Priority field location"
  validate_priority_mapping "$file"
  validate_colour_tables "$file"
  validate_styles "$file"
...
validate_dispatcher() {
  ...
  while IFS='|' read -r _ project_key routing_label project_number contract extra; do
    ...
    ((route_count += 1))
  done < <(awk '/^## Routes[[:space:]]*$/ { in_routes=1; next } in_routes && /^## / { exit } in_routes && /^\|/ { print }' "$file")
