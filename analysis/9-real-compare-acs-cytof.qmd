...
if (isTRUE(run_comparators)) {
  ...
  if (length(failed_comparator_pop_vec) > 0L) {
    failure_messages <- vapply(
      comparison_method_runs[failed_comparator_pop_vec],
      function(x) paste0(x$pop, ": ", x$error),
      character(1)
    )

    stop(
      "ACS Tailgate/F-beta runs failed:\n",
...
data("fn_to_sampleid_map", package = "DataTidyACSCyTOFFAUST")
...
manual_summary_tbl |>
  dplyr::filter(!method == "tailgate") |>
  dplyr::select(
    method:stim, pcc
  ) |>
  #dplyr::arrange(method:stim, pcc) |>
  tidyr::pivot_wider(
    names_from = method,
    values_from = pcc
  ) |>
  dplyr::mutate(
    diff_stimgate = stimgate - fbeta,
    diff_fbeta = fbeta - stimgate,
    diff_abs = pmax(abs(diff_stimgate), abs(diff_fbeta))
  ) |>
...
if (isTRUE(run_plots)) {
  p <- .acsCytofManualPlotScatter(manual_comparison_tbl)
  ...
  ggsave(path_p, plot = p, units = "cm", width = 20, height = 20)
  p <- .acsCytofManualPlotScatter(manual_comparison_tbl |> dplyr::filter(method != "tailgate"))
  path_p <- projr::projr_path_get(
    "cache",
    "acs_cytof",
    "manual-comparison",
...
if (isTRUE(run_plots)) {
  p <- .acsCytofManualPlotRelativeError(manual_comparison_tbl |> dplyr::filter(method != "tailgate"))
  path_p <- projr::projr_path_get(
    "cache",
    "acs_cytof",
    "manual-comparison",
